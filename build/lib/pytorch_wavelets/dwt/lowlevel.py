    ...
    if len(filts) == 2:
        ...
        if True in tensorize:
            h0_col, h1_col, h0_row, h1_row = prep_filt_afb2d(
                h0, h1, device=x.device, t=x.dtype)
        else:
            h0_col = h0
        ...
        if True in tensorize:
            h0_col, h1_col, h0_row, h1_row = prep_filt_afb2d(
                *filts, device=x.device, t=x.dtype)
        else:
    ...
    if isinstance(filts, (tuple, list)):
        if len(filts) == 2:
            filts = prep_filt_afb2d_nonsep(
                filts[0], filts[1], device=x.device, t=x.dtype)
        else:
            filts = prep_filt_afb2d_nonsep(
                filts[0], filts[1], filts[2], filts[3], device=x.device, t=x.dtype)
    ...
    if isinstance(filts, (tuple, list)):
        if len(filts) == 2:
            filts = prep_filt_sfb2d_nonsep(filts[0], filts[1],
                                           device=coeffs.device, t=coeffs.dtype)
        elif len(filts) == 4:
            filts = prep_filt_sfb2d_nonsep(
                filts[0], filts[1], filts[2], filts[3], device=coeffs.device, t=coeffs.dtype)
        else:
...
def prep_filt_afb2d_nonsep(h0_col, h1_col, h0_row=None, h1_row=None,
    ...
    filts = torch.tensor(filts, dtype=t, device=device)
    return filts
...
def prep_filt_sfb2d(g0_col, g1_col, g0_row=None, g1_row=None, device=None, t=torch.float):
    ...
    """
    g0_col, g1_col = prep_filt_sfb1d(g0_col, g1_col, device, t=t)
    if g0_row is None:
        g0_row, g1_row = g0_col, g1_col
    else:
        g0_row, g1_row = prep_filt_sfb1d(g0_row, g1_row, device, t=t)
...
def prep_filt_afb2d(h0_col, h1_col, h0_row=None, h1_row=None, device=None, t=torch.float):
    ...
    """
    h0_col, h1_col = prep_filt_afb1d(h0_col, h1_col, device, t=t)
    if h0_row is None:
        h0_row, h1_row = h0_col, h1_col
    else:
        h0_row, h1_row = prep_filt_afb1d(h0_row, h1_row, device, t=t)
